# Project Lattice: How I Understand The Product And Data Plan

## Core Product Thesis

The way I understand Project Lattice, this should not be just another automated valuation model. The wedge is explainable, evidence-backed real estate valuation: we estimate whether a property is fairly priced, overpriced, or undervalued, then show the sourced chain of reasons behind that judgment.
...
The right mental model for me is a real-estate knowledge graph plus agents. The graph stores typed entities and sourced relationships. The agents read and write that graph, but the graph and verifier are what make the product defensible.

## MVP Geography I Think We Should Consider
...
I would rather start with a graph that answers fewer questions very well than a broad graph that makes claims we cannot verify:
...
I want every Lattice explanation rejected unless it can pass these checks:
...
For a capstone-quality MVP, I would do this:
...
For a faster technical demo, I would do this:
