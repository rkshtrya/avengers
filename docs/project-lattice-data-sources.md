# Project Lattice: Product Understanding and Data Source Plan

## Core product thesis

Project Lattice is not just another automated valuation model. The product's wedge is explainable, evidence-backed real estate valuation: it estimates whether a property is fairly priced, overpriced, or undervalued, then shows the sourced chain of reasons behind that judgment.
...
The product should therefore optimize for three things at once:
...
The right mental model is a real-estate knowledge graph plus agents. The graph stores typed entities and sourced relationships. The agents read and write that graph, but the graph and verifier are what make the product defensible.

## Recommended MVP geography
...
Use this path if the demo story matters more than data convenience.
...
Use this path if shipping the six-week capstone is the priority. You can still tell the Lattice story and later port to San Ramon.
...
Start with a graph that can answer fewer questions very well:
...
Every Lattice explanation should be rejected unless it can pass these checks:
...
3. City permit and zoning coverage varies. The ingestion layer must store gaps explicitly instead of pretending coverage is complete.
4. OSM is useful but should not be the sole basis for high-stakes claims.
...
As of July 3, 2026, the source sufficiency is mixed:
...
The decision point is therefore not whether enough public data exists. It does. The decision point is whether the team can secure or seed enough comparable sales to make the valuation number credible.
