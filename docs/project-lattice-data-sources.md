...
This path works well if the demo story matters more than data convenience.
...
This path works well if shipping the six-week capstone is the priority. We can still tell the Lattice story and later port to San Ramon.
...
## The Source Strategy We Recommend

We can start with a graph that answers fewer questions very well rather than a broad graph that makes claims we cannot verify:
