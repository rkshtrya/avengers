...
def write_markdown(summaries: list[dict]) -> None:
    ...
    lines = []
    lines.append("# Project Lattice Data Handoff EDA Summary")
    lines.append("")
    lines.append("Generated from downloaded files in `data/raw` and metadata in `data/metadata`.")
    lines.append("")
    lines.append("## Executive Verdict")
    lines.append("")
    lines.append("- The handoff is strong enough for data-source review and first-pass EDA.")
    ...
    lines.append("2. **Risk layers are real and source-backed.** FEMA NFHL and CAL FIRE FHSZ services are queryable; the handoff includes Contra Costa bbox extracts.")
    ...
    lines.append("")
    lines.append("## Recommended Next EDA")
    lines.append("")
